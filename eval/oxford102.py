"""Oxford 102 Flowers: category names, the classes set aside as a gray zone, and the raw layout.

Shared by eval/build_test_set.py (the legacy 360-image sample) and eval/build_datasets.py
(the open-world evaluation pools, eval/PROTOCOL.md).
"""

from pathlib import Path

RAW_ROOT = Path(__file__).resolve().parent / "raw"
OXFORD_DIR = RAW_ROOT / "flowers-102"
...
# "barbeton daisy" in cat_to_name.json is a typo for "barberton daisy" (what species_mapping.py uses).
_CORRECTIONS = {"barbeton daisy": "barberton daisy"}

# Oxford categories with an unresolved taxonomic overlap with a covered species: excluded from BOTH the
# in-distribution and the near-OOD side (eval/PROTOCOL.md section 2.3).
GRAY_ZONE_CLASSES = frozenset(
    {
        "buttercup",  # not confirmed to be Ranunculus asiaticus (Persian buttercup)
        "windflower",  # not confirmed to be Anemone coronaria (Poppy anemone)
        "grape hyacinth",  # Muscari, not Hyacinthus (Hyacinth)
        "siam tulip",  # Curcuma, not Tulipa (Tulip)
        "bishop of llandaff",  # a Dahlia cultivar, and Dahlia is a covered species
    }
)


def class_name_by_label() -> dict[int, str]:
    """Oxford's 1-indexed category number (as in imagelabels.mat) -> category name."""
    return {int(number): _CORRECTIONS.get(name, name) for number, name in CAT_TO_NAME.items()}
