...
- **The `-latest` alias is not always the safe choice, and the 20-req/day quota isn't unique to one model.** `gemini-flash-latest` currently resolves to `gemini-3.8-flash`, released days before this was built, capped at **20 requests/day per project** — far below the ~1,500/day some older Flash generations reportedly get. BloomLens pinned to `gemini-3.6-flash` (`src/identify.py`) expecting more headroom, but hit the *same* 20/day cap during Milestone 2 testing — so this project/key's free tier appears to cap every current 3.x-generation model at 20/day, not just the newest one. The retry-after hint on the 429 (~20-30s, not hours) suggests it behaves more like a slow-refilling rate limit than a hard once-a-day wall, which is why Milestone 2's retry logic waits it out rather than failing outright. Worth re-checking as models age past their introductory quota, and worth testing an older/non-3.x model if quota becomes a real blocker.
